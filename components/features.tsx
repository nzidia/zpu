...
export default function Features() {
  ...
  const [baseColor, setBaseColor] = useState<[number, number, number]>([0.517, 0.761, 0.145]) // #84c225 in RGB normalized
  const [glowColor, setGlowColor] = useState<[number, number, number]>([0.517, 0.761, 0.145])

  const [dark, setDark] = useState<number>(theme === "dark" ? 1 : 0)

  useEffect(() => {
    setBaseColor([0.517, 0.761, 0.145]) // #84c225
    setGlowColor([0.517, 0.761, 0.145])
        ...
        <FollowerPointerCard
          title={
            <div className="flex items-center gap-2">
              <span>🔒</span>
              <span>Private Compute Infrastructure</span>
            </div>
          }
        >
          <div className="cursor-none">
            <div className="grid grid-cols-12 gap-4 justify-center">
              <motion.div
                ...
                whileHover={{
                  scale: 1.02,
                  borderColor: "rgba(132, 194, 37, 0.6)",
                  boxShadow: "0 0 30px rgba(132, 194, 37, 0.2)",
                }}
                style={{ transition: "all 0s ease-in-out" }}
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">ZPU Nodes</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      Decentralized compute providers that process AI and blockchain tasks privately using zkSNARKs.
                    </p>
                  </div>
                </div>
                <div className="pointer-events-none flex grow items-center justify-center select-none relative">
                  ...
                  >
                    ...
                    <motion.div
                      className="absolute top-1/2 left-1/2 w-16 h-16 bg-[#84c225] rounded-full blur-[74px] opacity-65 transform -translate-x-1/2 -translate-y-1/2"
                      initial={{ scale: 1 }}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="flex items-center gap-8">
                        <div className="flex flex-col gap-3">
                          {["Compute", "Verify", "Earn"].map((item, index) => (
                            <motion.div
                              key={`left-${index}`}
                        ...
                        >
                          <img src="/nzidia-logo.png" alt="NZIDIA" className="w-full h-full object-cover" />
                        </motion.div>

                        <div className="flex flex-col gap-3">
                          {["Private", "Fast", "Secure"].map((item, index) => (
                            <motion.div
                              key={`right-${index}`}
                              className="bg-white rounded px-3 py-2 flex items-center gap-2 text-black text-sm font-medium shadow-sm"
                              initial={{ opacity: 1, x: 0 }}
              ...
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">Global Privacy Network</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      Compute anywhere, stay private everywhere. Built on ZCash's battle-tested zkSNARK technology.
                    </p>
                  </div>
                </div>
                  ...
                  <h1 className="mt-8 text-center text-5xl leading-[100%] font-semibold sm:leading-normal lg:mt-12 lg:text-6xl">
                    <span className='bg-background relative mt-3 inline-block w-fit rounded-md border px-1.5 py-0.5 before:absolute before:top-0 before:left-0 before:z-10 before:h-full before:w-full before:bg-[url("/noise.gif")] before:opacity-[0.09] before:content-[""]'>
                      <ScrambleHover
                        ...
                        className="cursor-pointer bg-gradient-to-t from-[#84c225] to-[#84c225] bg-clip-text text-transparent"
              ...
              <motion.div
                ...
                whileHover={{
                  scale: 1.02,
                  borderColor: "rgba(132, 194, 37, 0.5)",
                  boxShadow: "0 0 30px rgba(132, 194, 37, 0.2)",
                }}
                style={{ transition: "all 0s ease-in-out" }}
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">ZAI Agents</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      Private AI bots powered by ZPU compute. Like ChatGPT, but your data never leaves the shadows.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="relative rounded-2xl border border-white/10 bg-black/20 dark:bg-white/5 backdrop-blur-sm">
                      ...
                      <div className="flex items-center justify-between px-4 pb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#84c225] hover:bg-[#84c225]/90 transition-colors text-white font-medium">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="16"
                              height="16"
              ...
              <motion.div
                ...
                whileHover={{
                  rotateY: 5,
                  rotateX: 2,
                  boxShadow: "0 20px 40px rgba(132, 194, 37, 0.3)",
                  borderColor: "rgba(132, 194, 37, 0.6)",
                }}
                style={{ transition: "all 0s ease-in-out" }}
              >
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl leading-none font-semibold tracking-tight">Proof of Compute</h3>
                  <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
                    <p className="max-w-[460px]">
                      Every computation generates verifiable zkProofs. Trust the math, not the node.
                    </p>
                  </div>
                </div>
                <div className="flex grow items-center justify-center select-none relative min-h-[300px] p-4">
